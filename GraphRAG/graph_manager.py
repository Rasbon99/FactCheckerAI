...
class GraphManager:
    ...
    def extract_and_save_graph(
        self, output_file_topic, output_file_entity, output_file_site
    ):
        """
        ...
        try:
            ...
            def create_and_save_graph(
                query, node_relation, node_label, edge_label, output_file
            ):
                """
                ...
                for _, row in results.iterrows():
                    G.add_edge(
                        row[node_relation[0]], row[node_relation[1]], label=edge_label
                    )

                # Generate a list of colors for the nodes
                colors = list(mcolors.TABLEAU_COLORS.values())
                color_map = {}
                unique_nodes = results[
                    node_relation[1]
                ].unique()  # Associate nodes with the second parameter of the relation
                for i, node in enumerate(unique_nodes):
                    color_map[node] = colors[
                        i % len(colors)
                    ]  # Recycle colors if nodes exceed available colors
                ...
                for u, v, data in G.edges(data=True):
                    edge_colors.append(
                        color_map[v]
                    )  # Arrow color based on the target node
                ...
                for node in G.nodes():
                    node_label = (
                        f"{node}"  # Or any other text to associate with the node
                    )
                    ...
                    while overlap:
                        ...
                        for i, node_i in enumerate(nodes):
                            for j, node_j in enumerate(nodes):
                                ...
                                dist = np.linalg.norm(
                                    np.array(pos[node_i]) - np.array(pos[node_j])
                                )
                                if dist < threshold:
                ...
                nx.draw_networkx_edge_labels(
                    G, pos, edge_labels=edge_labels, font_size=6, font_color="black"
                )
            ...
            create_and_save_graph(
                ...
                node_label="Topic",
                edge_label="HAS_TOPIC",
                output_file=output_file_topic,
            )
            ...
            create_and_save_graph(
                query_mentions,
                node_relation=("Article", "Entity"),
                node_label="Entity",
                edge_label="MENTIONS",
                output_file=output_file_entity,
            )
            ...
            create_and_save_graph(
                query_site,
                node_relation=("Article", "Site"),
                node_label="Site",
                edge_label="PUBLISHED_ON",
                output_file=output_file_site,
            )
