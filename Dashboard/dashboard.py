    ...
    def _initialize_session_state(self):
        ...
        if "messages" not in st.session_state:
            st.session_state["messages"] = [
                {"role": "assistant", "content": "To begin, state your claim..."}
            ]
    ...
    def run(self):
            ...
            if prompt:
                if self.is_numeric_claim(prompt):
                    self.display_message(
                        "assistant",
                        "This claim is invalid because it consists only of numbers.",
                    )
                    st.session_state.messages = []
                else:
        ...
        with st.sidebar:
            ...
            st.sidebar.text_input(
                "Search claims...", key="search_query", placeholder="Type to search"
            )
            ...
            filtered_conversations = (
                [
                    ...
                    if st.session_state.search_query.lower()
                    in convo.get("title", "").lower()
                ]
            ...
            for i, convo in enumerate(display_conversations):
                if st.sidebar.button(
                    convo["title"][:50] + ("..." if len(convo["title"]) > 50 else ""),
                    key=f"convo_{i}",
                ):
            ...
            if len(filtered_conversations) > max_display:
                if st.sidebar.button(
                    ...
                    if st.session_state.show_all_conversations
                    else "Show More Conversations"
                ):
                    st.session_state.show_all_conversations = (
                        not st.session_state.show_all_conversations
                    )
                    st.rerun()
